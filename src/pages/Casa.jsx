...
import { useTarefasCasa, useFrentes, useResponsive } from '../hooks/useSupabase'
import { useAuth } from '../contexts/AuthContext'
import { useUI } from '../contexts/UIContext'
import { CASA_CONFIG } from '../lib/workspace-config'
import { FRENTES_CASA_CORES, DONOS_CASA, splitDonos } from '../lib/constants'
...
export default function CasaPage({ view = 'dashboard' }) {
  ...
  // ─── Derived Data ──────────────────────────────
  const frenteNames = frentesCasa.length > 0 ? frentesCasa.map(f => f.nome) : Object.keys(FRENTES_CASA_CORES)
  const frenteCores = frentesCasa.length > 0 ? Object.fromEntries(frentesCasa.map(f => [f.nome, f.cor])) : FRENTES_CASA_CORES
    ...
    <div className="min-h-screen">
      ...
      <div className={pad}>
        <FilterBar filters={filters} setFilter={setFilter} isMobile={isMobile}
          ...
          donoOptions={['Todos', ...(allDonos.length > 0 ? allDonos : DONOS_CASA)]} />
      </div>
